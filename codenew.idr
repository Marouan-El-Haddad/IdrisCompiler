...
data Exp : (vEnv: Vect n TyExp) -> (fEnv: (TyExp, TyExp)) -> TyExp -> Type where
  ...
  ExpFuncCall: Exp vEnv (s,t) t -> Exp vEnv (s,t) t
  --ExpFuncCall: (argType : fEnv.fst = s) -> Exp vEnv (s,t) s -> Exp vEnv (s,t) t
  --In this example, argType is a type constraint that checks that the first element of fEnv (which is the input type of the function) is equal to s. If the constraint is satisfied, then the ExpFuncCall constructor can be used. Otherwise, a type error will be raised.
  ---> ExpFuncCall : (argType : fEnv.fst = s) -> Exp vEnv (s,t) t -> Exp vEnv (s,t) t
  --In this example, the type of x in the ExpFuncCall constructor has been changed from Exp vEnv (s,t) s to Exp vEnv (s,t) t, so that it matches the expected type of ?holex, which is Exp vEnv (s,t) t.
                ...
                = evalOpenProg (
                    MkOpenProgram p_funDecl (p_funDecl.fd_return_type) p_funDecl.fd_var_type (
                      evalProg (assert_smaller x $
                        MkProgram p_funDecl (p_funDecl.fd_var_type) ?hole
                        )
                    ) p_funDecl.body 
                  )
...
--Define a fibonacci function that returns the nth fibonacci number
fib : FunDecl
fib = MkFunDecl Tint Tint (ExpIfThenElse (ExpLessThan (ExpVar (StopVar)) (ExpVal 2)) (ExpVar (StopVar)) (ExpAddition (ExpFuncCall (ExpVar (StopVar))) (ExpFuncCall (ExpVar (StopVar)))))

-- Create an OpenProgram value that includes the fib function and the necessary return type
prog6 : OpenProgram
prog6 = MkOpenProgram fib Tint Tint 3 (ExpIfThenElse (ExpLessThan (ExpVal 3) (ExpVal 2)) (ExpVal 3) (ExpAddition (ExpFuncCall (ExpVal 3)) (ExpFuncCall (ExpVal 3))))
